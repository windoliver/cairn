...
pub struct ParsedProjection {
    ...
    pub raw_frontmatter: BTreeMap<String, serde_yml::Value>,
}
...
impl MarkdownProjector {
    ...
    pub fn project(&self, stored: &StoredRecord) -> ProjectedFile {
        ...
        let mut yaml_map = serde_yml::to_value(&doc)
        ...
        for (k, v) in &r.extra_frontmatter {
            ...
            if !PROJECTED_STANDARD_FIELDS.contains(&k.as_str())
                && let Ok(yaml_val) = serde_yml::to_value(v)
            {
                yaml_map.insert(serde_yml::Value::String(k.clone()), yaml_val);
            }
        }
        #[allow(clippy::expect_used)]
        let yaml = serde_yml::to_string(&yaml_map).expect("YAML mapping serializes infallibly");
        ...
        debug_assert!(
            !yaml.starts_with("---\n"),
            "serde_yml now prepends document-start marker; strip_prefix logic needs revisiting: {:?}",
    ...
    pub fn parse(&self, content: &str) -> Result<ParsedProjection, ResyncError> {
        ...
        let val: serde_yml::Value =
            serde_yml::from_str(yaml_part).map_err(|e| ResyncError::ParseFailed(e.to_string()))?;
    ...
    pub fn check_conflict(
        ...
        // the same serde_yml round-trip. This avoids f32↔f64 precision mismatches and
