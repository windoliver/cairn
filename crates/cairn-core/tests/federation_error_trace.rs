...
fn each_sharing_overlap_maps_to_expected_kind() {
    ...
    assert_eq!(
        E::Expired.sharing_decision(),
        Some(SharingDecisionKind::Expired)
    );
    assert_eq!(
        E::TargetMismatch.sharing_decision(),
        Some(SharingDecisionKind::TargetMismatch)
    );
    assert_eq!(
        E::ScopeMismatch.sharing_decision(),
        Some(SharingDecisionKind::ScopeMismatch)
    );
    assert_eq!(
        E::TierMismatch.sharing_decision(),
        Some(SharingDecisionKind::TierMismatch)
    );
    assert_eq!(
        E::BadSignature.sharing_decision(),
        Some(SharingDecisionKind::BadSignature)
    );
    assert_eq!(
        E::Revoked.sharing_decision(),
        Some(SharingDecisionKind::Revoked)
    );
    assert_eq!(
        E::NotHuman.sharing_decision(),
        Some(SharingDecisionKind::NotHuman)
    );
    assert_eq!(
        E::NoRebacRelation.sharing_decision(),
        Some(SharingDecisionKind::NoRebacRelation)
    );
    assert_eq!(
        E::InvalidShape.sharing_decision(),
        Some(SharingDecisionKind::InvalidShape)
    );
}
...
fn display_messages_are_body_free() {
    ...
    assert_eq!(
        E::CapabilityDisabled.to_string(),
        "federation capability not advertised"
    );
}
