...
use super::skill_compat::{
    any_of_required_branches, discriminator_const, required_excluding_const,
};
...
fn required_args_per_command(verb: &VerbDef, cmds: &[&CliCommand]) -> Vec<BTreeSet<String>> {
    ...
    // Tagged-union: walk oneOf and pull required from each $defs/<Variant>,
    // promoting the discriminator flag and the first `anyOf` branch into the
    // required set so the generated example mirrors the matcher's view.
    if let Some(one_of) = args.get("oneOf").and_then(serde_json::Value::as_array) {
        let mut out = Vec::with_capacity(cmd_count);
        for (idx, entry) in one_of.iter().enumerate() {
            let variant_schema = entry
                .get("$ref")
                .and_then(serde_json::Value::as_str)
                .and_then(|p| p.strip_prefix("#/$defs/"))
                .and_then(|name| defs.get(name));
            let mut required = variant_schema
                .map(required_excluding_const)
                .unwrap_or_default();
            if let Some(vs) = variant_schema {
                if let Some(disc) = discriminator_const(vs)
                    && let Some(cmd) = cmds.get(idx)
                    && let Some(flag) = cmd.flags.iter().find(|f| f.long == disc || f.name == disc)
                {
                    required.insert(flag.name.clone());
                }
                if let Some(first) = any_of_required_branches(vs).into_iter().next() {
    ...
    // Single-shape: required lives on $defs/Args itself.
    let mut out = Vec::with_capacity(cmd_count);
    out.push(required_excluding_const(args));
    out.resize(cmd_count, BTreeSet::new());
    out
}
