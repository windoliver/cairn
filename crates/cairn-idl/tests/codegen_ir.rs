...
use cairn_idl::codegen::ir::{Ctx, Prim, RustType, TypeName, build, lower_schema};
...
fn array_of_strings() {
    ...
    let mut ctx = Ctx::default();
    let ty = lower_schema(&v, &mut ctx).unwrap();
    let RustType::Vec(inner) = ty else {
        panic!("expected Vec, got {ty:?}")
    };
...
fn tagged_union_with_discriminator() {
    ...
    let ty = lower_schema(&v, &mut ctx).unwrap();
    let RustType::TaggedUnion(t) = ty else {
        panic!("expected TaggedUnion, got {ty:?}")
    };
    ...
    assert_eq!(
        t.variants[1].capability.as_deref(),
        Some("cairn.mcp.v1.retrieve.session")
    );
}
...
fn filter_family_collapses_to_recursive() {
    ...
    let RustType::Recursive(r) = ty else {
        panic!("expected Recursive, got {ty:?}")
    };
