...
/// # Errors
///
/// Returns [`McpTransportError::Initialize`] if the rmcp service fails to
/// initialize (e.g. the client sends a malformed `initialize` request), or
/// [`McpTransportError::Service`] if the service terminates abnormally. Normal
/// stdin-EOF shutdown is not an error and returns `Ok(())`.
///
