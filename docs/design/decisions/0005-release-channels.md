...
- **Payload is HTTP-standard, no Cairn-controlled identifiers.** The
  ...
  Cairn-controlled beyond that. The host can infer coarse
  location/region from the requester IP (HTTP-standard; outside
  Cairn's control). The `CLAUDE.md` §6.6 rule ("never log raw record
  bodies above `debug`") is extended here to also cover update-poll
  activity.
...
cairn` — are independent of `update.channel` and walk whichever tap
or source the user installed. To switch the CLI channel via a
package manager, the user must also switch the relevant tap (e.g.
`brew tap cairn/beta && brew install cairn` for the beta CLI). The
desktop binary and the CLI binary on the same machine can be on
different channels.
