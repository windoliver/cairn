...
| §8 CLI / MCP / SDK / skill contract | #9, #10, #11, #53, #59–#70, #98 | — | IDL-generated surfaces and parity checks covered. #53 lands capability advertisement parity: `cairn-core::status::advertise` is the single source of truth read by all four surfaces, with `REMEDIATION` hints kept in sync. #65 lands MCP `initialize` ↔ `status` parity (Cairn `StatusResponse` rides in `serverCapabilities.experimental["cairn.status"]`) and exposes `handshake` as an MCP prelude tool that persists fresh nonces to `outstanding_challenges`; extension namespaces stay absent from `tools/list` under default config (brief §8.0.a). #67 lands the MCP conformance suite: fixture loader, envelope canonicaliser, `conformance_envelope_replay` runner with bless workflow, happy-path + gap-fill fixtures for all v0.1 verbs, JSON-RPC outer-envelope well-formedness check, and cross-product backstop; test entry point is `crates/cairn-mcp/tests/conformance.rs`. #98 lands the wire-compat + capability-matrix release gate (`cairn.mcp.v1` schema fingerprint, per-file snapshots, v0.1 `advertise()` matrix, single `contract-drift` CI job). ADR 0004 codifies the v1.0 `cairn.mcp.v1` semver freeze (frozen surface, additive/deprecation/v2 rules, extension-namespace policy) and names `contract-drift` as the release-blocking enforcement gate. |
...
| §12.a Distribution model | #26, #29, #121–#123, #130–#132 | #149 (open) | ReBAC, share links, propagation workflow + `cairn.federation.v1` extension protocol (capability still gated until MCP runtime wires `Keystore` + `FederationTransport` + `FederationOutbox` into `ServerState`), connectors, aggregate memory; HTTP transport adapter is a follow-up. |
...
| §15 Evaluation | #18, #24, #31, #97–#100, #116–#118, #136–#138 | `docs/design/2026-05-24-coherence-benchmarks-design.md` | P0 replay and gates, P1 bench and SRE, v0.4 cassette and doc freeze, v0.4 coherence gate (#137), v0.4 docs freeze landed (#138). |
| §16 Packaging | #18, #32, #100, #139–#142, #158 | ADR 0005 (resolved) | Cargo and Homebrew, static smoke tests, desktop production packaging, release channels. #141 lands ADR 0005 (release channels + auto-update policy) — stable/beta/nightly matrix, per-OS update mechanism, Cosign signing scheme, off-by-default privacy contract, bidirectional channel migration with vault-schema-downgrade guard, manual rollback. Implementation of signing infra / electron-updater wiring / `cairn release verify` is tracked as named follow-ups under parent epic #32, each citing ADR 0005. |
...
| §18 Success criteria / adoption / consumer blueprint | #11, #18, #19, #25, #31, #32, #68–#70, #97–#103, #119–#120, #138–#142, #343 | #136, #137 | User stories, skill adoption, reference consumers, and production criteria. #343 adds the zero-capture reminder decision slice for the v0.1 reference consumer. #138 lands v0.4 docs freeze + consumer-blueprint migration guides + beta readiness checklist. |
| §18.d Cairn skill | #11, #68–#70, #182 | — | Install, conventions, compatibility checks. #182 introduces the `cairn-pack/v1` harness-pack manifest schema (distinct from the skillify `SkillPackManifest`) and ships `packs/cairn-claude-code/` as the reference pack — six MCP-only subagents, 13 slash commands, five-event hook bindings, manual fragment, and a dogfood fixture vault. The pack runtime lives in `crates/cairn-cli/src/packs/` (embed + Pass A/B validate + install + verify). Migrating the inline Claude-Code content out of `crates/cairn-cli/src/skill.rs` restored the harness-agnostic core invariant (CLAUDE.md §4 invariant 1). |
| §19 Sequencing | _none_ | ADR 0004 (resolved) | Process-only row; phased milestones and `priority:` / `phase:` labels encode the sequence. |
