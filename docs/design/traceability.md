...
| §4.1 MCPServer (auth substrate) | #190 | — | Plan A — `[mcp.stdio]` config (`single_tenant`, `principal`), `McpSessionScope` trait + `ConfigBackedScope`, `mcp_graph_tools_available` predicate, `serve_stdio_with_store`, CLI scope-resolution wiring, and `mcp.graph_tools` status probe. |
