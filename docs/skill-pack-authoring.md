...
`schema: "cairn-pack/v1"` in `pack.json` and are verified or installed by
`cairn plugins verify --pack-path`. These packs are distinct from Skillify
`.cairnpack` archives managed by `cairn skillpack`; archive generation and
Skillify packaging have their own lifecycle and are not the authoring format
...
pack-relative path and the exact `uses_mcp_tools` allowlist. The prompt
frontmatter must match the manifest allowlist so the runtime and verifier agree
on which tools the subagent may call.
...
location expected by the install target: `CLAUDE.md` for Claude-style harness
manuals, `AGENTS.md` for Codex, and `GEMINI.md` for Gemini. The starter scaffold
chooses the correct fragment path for its selected harness.
...
capabilities, frontmatter drift, unknown hook events, and failed smoke scripts
all indicate the pack is not portable.
...
frontmatter, hook, command, or manual-fragment change. For release candidates,
run the verifier from a clean checkout so generated or untracked local files do
not mask missing pack content.
